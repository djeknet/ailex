...
export default function MessageList({ isFullscreen = false }: MessageListProps) {
  ...
  const handleRetry = async (messageId: string) => {
    try {
      ...
      // Если это был site prompt с capabilities, нужно получить данные заново
      let pageContext: string | undefined;
      let webSearchEnabled = message.webSearch || false;
      const capabilities = message.sitePromptCapabilities || [];

      if (capabilities.length > 0) {
        console.log('[MessageList] Retrying site prompt with capabilities:', capabilities);
        ...
        if (tab?.id) {
          ...
          if (capabilities.includes('context')) {
            try {
              ...
              if (response?.success && response.data) {
                ...
                console.log('[MessageList] Got page context for retry:', pageContext?.length || 0, 'chars');
              }
            } catch (error) {
              console.warn('[MessageList] Could not get page context for retry:', error);
            }
          }
          ...
          if (capabilities.includes('transcribe_youtube')) {
            try {
              ...
              if (response?.success && response.result) {
                ...
                console.log('[MessageList] YouTube transcript added for retry:', transcript.length, 'chars');
              } else {
                console.warn('[MessageList] Could not get YouTube transcript for retry:', response?.error);
              }
            } catch (error) {
              console.warn('[MessageList] Error getting YouTube transcript for retry:', error);
            }
          }
          ...
          if (capabilities.includes('vision')) {
            try {
              ...
              console.log('[MessageList] Screenshot captured for retry');
            } catch (error) {
              console.warn('[MessageList] Error capturing screenshot for retry:', error);
            }
          }
      ...
      await sendUserMessage(
        message.text,
        pageContext, // передаем полученный контекст или undefined
        message.replyTo,
        message.actionLabel,
        attachments,
        webSearchEnabled,
        ...
        message.sitePromptCapabilities, // передаем capabilities
        message.sitePromptTextKey // передаем textKey
      );
    } catch (error) {
  ...
  const handleRewrite = async (messageId: string, action: string) => {
    try {
      ...
      // Передаем пустую строку как pageContext, чтобы отключить автоматическое получение контекста
      // (для rewrite контекст страницы не нужен)
      await sendUserMessage(fullPrompt, '', messageId, actionLabel);
