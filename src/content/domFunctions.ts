...
export interface YouTubeTranscribeParams {
  language?: string; // Предпочитаемый язык (по умолчанию 'en')
  autoGenerated?: boolean; // Разрешить автогенерированные субтитры
}

/**
 * Принудительно включает субтитры, чтобы заставить плеер YouTube
 * отправить сетевой запрос (который перехватит background script)
 */
function forceTranscriptRequest(): boolean {
  console.log('[youtubeTranscribe] Attempting to force subtitle request...');
  
  // Используем селектор для кнопки субтитров
  const button = document.querySelector('button.ytp-subtitles-button');
  
  if (!button) {
    console.warn('[youtubeTranscribe] Subtitle button not found');
    return false;
  }
  
  // Проверяем, включены ли субтитры уже
  const isPressed = button.getAttribute('aria-pressed') === 'true';
  
  if (isPressed) {
    console.log('[youtubeTranscribe] Subtitles already enabled, request should have been sent');
    return true;
  }
  
  // Кликаем по кнопке
  (button as HTMLButtonElement).click();
  console.log('[youtubeTranscribe] Clicked subtitle button, waiting for network request...');
  
  return true;
}

/**
 * Получить текстовую расшифровку (transcript) видео YouTube
 * Использует перехваченный background скриптом URL субтитров
 */
export async function youtubeTranscribe(_params: YouTubeTranscribeParams = {}): Promise<string> {
  try {
    console.log('[youtubeTranscribe] Starting...');
    
    // 1. Принудительно включаем субтитры для отправки XHR запроса
    const buttonClicked = forceTranscriptRequest();
    
    if (buttonClicked) {
      // Даём время на отправку XHR запроса (обычно ~500-1000ms)
      console.log('[youtubeTranscribe] Waiting 1.5s for XHR request...');
      await new Promise(resolve => setTimeout(resolve, 1500));
    }
    
    // 2. Запрашиваем XML субтитров от background script
    console.log('[youtubeTranscribe] Requesting transcript from background...');
    const response = await chrome.runtime.sendMessage({
      type: 'GET_YOUTUBE_TRANSCRIPT'
    });
    
    if (!response?.success) {
      console.error('[youtubeTranscribe] Failed to get transcript:', response?.error);
      throw new Error(response?.error || 'Failed to get transcript from background');
    }
    
    const responseText = response.xmlText;
    
    if (!responseText || responseText.length === 0) {
      console.error('[youtubeTranscribe] Empty response received');
      throw new Error('Empty response');
    }
    
    console.log('[youtubeTranscribe] Received response, length:', responseText.length);
    console.log('[youtubeTranscribe] First 200 chars:', responseText.substring(0, 200));
    
    let transcript = '';
    
    // 2. Пробуем JSON3 формат
    if (responseText.trim().startsWith('{')) {
      console.log('[youtubeTranscribe] Parsing as JSON3...');
      try {
        const jsonData = JSON.parse(responseText);
        console.log('[youtubeTranscribe] JSON parsed, events:', jsonData.events?.length);
        ...
        if (jsonData.events && Array.isArray(jsonData.events)) {
          ...
          for (const event of jsonData.events) {
            if (event.segs && Array.isArray(event.segs)) {
              // Собираем текст из всех сегментов события
              const eventText = event.segs
                .map((seg: any) => seg.utf8 || '')
                .join('')
                .trim();
              
              // Игнорируем события, содержащие только перенос строки
          ...
          console.log('[youtubeTranscribe] Extracted from JSON3:', textParts.length, 'segments, total length:', transcript.length);
          
          // Если успешно получили транскрипт, сразу возвращаем его
          if (transcript) {
            console.log('[youtubeTranscribe] Final transcript length:', transcript.length, 'chars');
            return transcript;
          }
        }
      } catch (jsonError) {
        console.warn('[youtubeTranscribe] Failed to parse as JSON3:', jsonError);
      }
    }
    
    // 3. Если JSON не сработал, пробуем XML
    if (!transcript) {
      console.log('[youtubeTranscribe] Parsing as XML...');
      const parser = new DOMParser();
      const xmlDoc = parser.parseFromString(responseText, 'text/xml');
      
      // Извлекаем все <text> теги
      const textElements = xmlDoc.getElementsByTagName('text');
      console.log('[youtubeTranscribe] Found', textElements.length, 'text elements in XML');
      
      if (textElements.length === 0) {
        console.error('[youtubeTranscribe] No text elements in XML');
        console.error('[youtubeTranscribe] XML structure:', xmlDoc.documentElement?.tagName);
        throw new Error('No text elements found in subtitles XML');
      }
      
      // Собираем текст (textContent автоматически декодирует HTML entities)
      ...
      console.log('[youtubeTranscribe] Extracted from XML, length:', transcript.length);
    }
    
    if (!transcript) {
      console.error('[youtubeTranscribe] Failed to extract any text');
    ...
    console.error('[youtubeTranscribe] Error:', error);
    // Возвращаем пустую строку вместо выброса ошибки
    return '';
  }
}

// ============================================================================
// 7. Структурные и утилитарные функции
// ============================================================================
