 ...
 */
export async function youtubeTranscribe(params: YouTubeTranscribeParams = {}): Promise<string> {
  const { language = 'en', autoGenerated = true } = params;
  
  try {
    // 1. Ищем ytInitialPlayerResponse в DOM
    ...
    if (!playerResponse) {
      throw new Error(chrome.i18n.getMessage('youtubePlayerResponseNotFound'));
    }
    
    // 2. Получаем captionTracks
    const captionTracks = playerResponse?.captions?.playerCaptionsTracklistRenderer?.captionTracks;
    
    if (!captionTracks || captionTracks.length === 0) {
      throw new Error(chrome.i18n.getMessage('youtubeSubtitlesNotFound'));
    }
    
    // 3. Выбираем нужный трек
    let selectedTrack = captionTracks.find((track: any) => 
      track.languageCode === language && (autoGenerated || !track.kind)
    );
    
    // Если не нашли нужный язык, берём первый доступный
    if (!selectedTrack) {
      selectedTrack = captionTracks[0];
    }
    
    if (!selectedTrack?.baseUrl) {
      throw new Error(chrome.i18n.getMessage('youtubeSubtitlesBaseUrlNotFound'));
    }
    
    // 4. Загружаем XML субтитров
    const response = await fetch(selectedTrack.baseUrl);
    const xmlText = await response.text();
    
    // 5. Парсим XML и извлекаем текст
    ...
    console.error('Error in youtubeTranscribe:', error);
    throw error;
  }
}
